function B_i_sol_t = solve_diffEqu_B(rod,start_time, H_i_history,...
    H_i_dot_history, iteration, time_step, H_c, H_r, B_m,...
    initial_conditions)
...
end_time = start_time + iteration*time_step;
time_interval = linspace(start_time, end_time, iteration);
%% Adjusting Histories
...
if iteration ~= 1
    [t, B_i_sol] = ode45(@(t,B) get_dB_dt(t, B, time_interval, H_c, H_r,...
        B_m, H_i_history, H_i_dot_history), [0 end_time], initial_conditions);
else
    dB_dt = @(t,B) ((2*B_m)/(H_r*pi))*((((H_c - H_i_history(1))*cos((pi*B)...
        /(2*B_m)) + H_r*sin((pi*B)/(2*B_m)))/(2*H_c))^2)...
        * H_i_dot_history(1);
    [t, B_i_sol] = ode45(dB_dt, [0 end_time], initial_conditions);
end
